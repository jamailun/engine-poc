...
class Quadtree {
using GetBoxFunction = std::function<Box<Float>(const T&)>;
using EqualityFunction = std::function<bool(const T&, const T&)>;
...
public:
    Quadtree(const Box<Float>& box, const GetBoxFunction& getBox, const EqualityFunction& equal) :
    ...
    void debug_draw() const {
        engine::drawer::draw_rect(
            engine::math::Rect(mBox.left, mBox.top, mBox.width, mBox.height),
            engine::colors::red
        );
    }
...
private:
    ...
    GetBoxFunction mGetBox;
    EqualityFunction mEqual;
